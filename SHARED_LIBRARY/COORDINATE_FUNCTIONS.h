#ifndef COORDINATE_FUNCTIONS_H
#define COORDINATE_FUNCTIONS_H
...
void formatHelixWhiteCanvas(THelix *h, float pz, float pt, int color, bool doWTA) {
  ...
  // Setting line attributes
  if (color == 0) h->SetLineColor(kSpring);
  if (color == 1) h->SetLineColor(kWhite);
  if (color == 2) h->SetLineColor(kMagenta);
  if (color == 3) h->SetLineColor(kRed);
  if (color == 4) h->SetLineColor(kOrange+7);
  if (color == 5) h->SetLineColor(kAzure-3);
  if (color == -1) h->SetLineColor(kBlack);
  h->SetLineWidth(1);
